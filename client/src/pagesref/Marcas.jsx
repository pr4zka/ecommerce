...
const Marcas = () => {
  const { getMarca, marcas, handleDeleteMarca } = useTasks();
      ...
      <div className="mt-12">
        ...
        <table className="w-4/6 table-auto">
          ...
          <tbody>
            {marcas.map((marca) => (
                        <tr key={marca.id}>
                            <td className="border pl-2 border-r-indigo-500 border-l-indigo-500">{marca.id}</td>
                            <td className="border pl-2 border-r-indigo-500">{marca.descripcion}</td>
